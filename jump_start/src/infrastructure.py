...
class DNSContainer(InfraCont):
    ...
    def generate_config(self):
        ...
        for records in dns_zone['records']:
            rdtyp = dns.rdatatype.from_text(record['type'].upper())
            record_data = dns.rdata.from_text(dns.rdataclass.IN, rdtype, ' '.join(record['data']))
            zone.get_rdataset(record['name'], rdtype, create=True)
        with open(self.mnt_vol + '/' + dns_zone['name'] + '.db', 'w') as zone_conf:
